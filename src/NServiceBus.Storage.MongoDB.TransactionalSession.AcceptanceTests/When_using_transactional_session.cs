...
public class When_using_transactional_session : NServiceBusAcceptanceTest
{
    ...
    [TestCase(true)]
    [TestCase(false)]
    public async Task Should_send_messages_and_store_document_in_synchronized_session_on_transactional_session_commit(
        bool outboxEnabled)
    {
    ...
    [TestCase(true)]
    [TestCase(false)]
    public async Task Should_send_messages_and_store_document_in_mongo_session_on_transactional_session_commit(
        bool outboxEnabled)
    {
        var context = await Scenario.Define<Context>()
            .WithEndpoint<AnEndpoint>(s => s.When(async (_, ctx) =>
            {
                ...
                await mongoSession.MongoSession.Client.GetDatabase(SetupFixture.DatabaseName)
                    .GetCollection<SampleDocument>(CollectionName)
                    .InsertOneAsync(mongoSession.MongoSession,
                        new SampleDocument { Id = transactionalSession.SessionId });
    ...
    [TestCase(true)]
    [TestCase(false)]
    public async Task Should_allow_using_synchronized_storage_even_when_there_are_no_outgoing_operations(
        bool outboxEnabled)
    {
        var context = await Scenario.Define<Context>()
            .WithEndpoint<AnEndpoint>(s => s.When(async (statelessSession, ctx) =>
